# 크리스마스 프로모션
- 우테코 식당에서 12월 동안의 주문에 대해 할인 및 각종 이벤트를 진행하고 결과를 출력합니다.
- 진행하는 이벤트는 다음과 같습니다.
  - 크리스마스 디데이 할인
...
- [x] 주문할 메뉴와 개수를 입력받는다.
  ...
  - [x] [예외처리]주문 `메뉴`의 종류가 음료 밖에 없을 시 예외를 발생시킨다.
  - [x] [예외처리]`개수`의 총 합이 `20개`가 넘을 경우 예외를 발생시킨다.
  - [x] [예외처리]`개수`가 1보다 작은 경우 경우 예외를 발생시킨다.
  - [x] [예외처리]`개수`가 숫자가 아닌 경우 예외를 발생시킨다.
