...
## 참고 사항
- 증정 메뉴(샴페인)이 지급될 경우
  - `<할인 전 총주문 금액>`은 증정 메뉴에 영향을 받지 않습니다.
  - `<혜택 내역>`에 증정 메뉴의 가격이 기입됩니다.
  - `<총혜택 금액>`에는 증정 메뉴의 가격까지 포함됩니다.
  - 따라서 `<할인 후 예상 결제 금액>`은 `<할인 전 총주문 금액>`에서 증정 이벤트를 제외한 `<총혜택 금액>`만큼 할인된 가격입니다.
  - 따라서 `혜택`이라는 큰 범주 안에 `할인`과 `증정`이 포함됩니다.
---
...
- [x] 주문할 메뉴와 개수를 입력받는다.
  ...
  - [x] `메뉴-개수` 형식으로 입력받는다.
  - [x] 쉼표로 구분된 `메뉴-개수` 형식의 앞 뒤 공백을 제거한다.
  - [x] [예외처리]준비되지 않은 메뉴를 입력할 시 예외를 발생시킨다.
  - [x] [예외처리]주문 메뉴 개수가 숫자가 아닌 경우 예외를 발생시킨다.
  - [x] [예외처리]주문 메뉴가 1보다 작은 경우 경우 예외를 발생시킨다.
  - [x] [예외처리]주문 메뉴 개수의 총 합이 `20개`가 넘을 경우 예외를 발생시킨다.
  - [x] [예외처리]주문 메뉴에 음료밖에 없을 시 예외를 발생시킨다.
...
- [x] 주문들을 종합해 이벤트 적용 결과를 만든다.
  ...
  - [x] 총주문 금액을 보고 샴페인 증정 여부를 결정한 뒤 할인가에 반영한다.
  - [x] 주문들을 보고 혜택 내역을 결정한다.
  - [x] 총주문 금액과 총 혜택 내역을 보고 할인 후 예상 결제 금액을 계산한다.
  - [x] 할인 후 총 예상 결제 금액을 보고 12월 이벤트 배지를 결정한다.
